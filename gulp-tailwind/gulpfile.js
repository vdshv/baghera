...
function devStyles(){
  ...
  return src(`${options.paths.src.css}/**/*.scss`).pipe(sass().on('error', sass.logError))
    ...
    .pipe(dest(options.paths.shopify.base));
}
...
function watchFiles(){
  ...
  watch(`${options.paths.shopify.liquid_sections}/*.liquid`,series(devStyles, previewReload));
  watch(`${options.paths.shopify.liquid_snippets}/*.liquid`,series(devStyles, previewReload));
